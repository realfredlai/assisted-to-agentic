...
- **Run every command you put in a doc before committing it.** A command that has never been executed and one that works look identical on the page — and when it fails, the reader blames the software, not the documentation. (Added after work item 006, where a plausible-looking MCP Inspector invocation in a README had never been run and did not work.)
