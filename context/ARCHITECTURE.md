...
```
knowledge/*.yaml ──make knowledge-import──► knowledge.db ──direct import──► MCP tools ──stdio──► agent
                        (Django + PyYAML)     (SQLite)      (stdlib only)
```

| Tool | Returns |
|------|---------|
| `lookup_term` | definition, aliases, **warnings**, source files, docs — by id, name, or alias, case-insensitively |
| `get_related_terms` | outgoing edges, each with the target's id *and* display name |
...
- **18 tests**, hermetic by construction: fixtures build graphs in-process with `Storage`, never by shelling out to `manage.py`, so `make mcp-test` passes with Docker stopped and `knowledge.db` absent. Split as protocol layer (`main_test.py`) and tool behaviour (`tools_test.py`).
